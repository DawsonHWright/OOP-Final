...
class Board(Drawable):
    ...
    def __init__(self, isBig: bool):
        if isBig:
            self._children: Drawable = [[Board(False) for _ in range(3)] for _ in range(3)]
        else:
            self._children: Drawable = [[Square() for _ in range(3)] for _ in range(3)]
        self.status = -1
        self.isBig = isBig
    ...
    def isWon(self) -> int:
        ...
        for i in range(3):
            # Rows
            if all(get_status(self._children[i][j]) == get_status(self._children[i][0]) != -1 for j in range(3)):
                return get_status(self._children[i][0])
            # Columns
            if all(get_status(self._children[j][i]) == get_status(self._children[0][i]) != -1 for j in range(3)):
                return get_status(self._children[0][i])

        # Diagonals
        if all(get_status(self._children[i][i]) == get_status(self._children[0][0]) != -1 for i in range(3)):
            return get_status(self._children[0][0])
        if all(get_status(self._children[i][2-i]) == get_status(self._children[0][2]) != -1 for i in range(3)):
            return get_status(self._children[0][2])

        # Stalemate
        if all(get_status(self._children[i][j]) != -1 for i in range(3) for j in range(3)):
            self.status = 2
            return 2  # Draw
        return -1

    def draw(self, surface: Surface, margin: float, play_mode: int = 0, playable: (int, int) = None) -> None:
        size_x, size_y = surface.get_size()
        my_margin = int(margin * size_x)

        # Working area, leaving margin on the outside
        working_rect = surface.get_rect().inflate(-2 * my_margin, -2 * my_margin)
